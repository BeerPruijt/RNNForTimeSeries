...
def generate_dummy_series(dataset='dummy', log_diff=False):
    """
    df
        Month  Passengers
    0    1949-01         112
    ...
    if dataset == 'linear':
        df.loc[:, 'Passengers'] = [i/2 for i in range(len(df))] # Linear trend
    elif dataset == 'sinus':
        df.loc[:, 'Passengers'] = [np.sin(i/2) for i in range(len(df))] # Sinusoidal trend

    start_idx = 0
    if log_diff:
        df.loc[:, 'Passengers'] = np.log(df.loc[:, 'Passengers']).diff() 
        start_idx = 1

    timeseries_original = df.loc[df.index[start_idx::], ["Passengers"]].values.astype('float32')
